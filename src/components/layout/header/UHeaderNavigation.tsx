...
const UHeaderNavigation = () => {
  ...
  const handleUserClick = () => {
    if (role.startsWith("Student")) router.push("/student/profile");
    else if (role.startsWith("Company")) router.push("/enterprise/profile");
  };
  ...
  return (
    <nav className={"z-10 flex gap-4 items-center"}>
      ...
      {/* Role */}
      {isLoggedIn && (
        <span className="rounded-2xl px-4 py-2 text-sm font-medium shadow-lg bg-custom-black/90 text-custom-yellow-3/90">
          {role}
        </span>
      )}
