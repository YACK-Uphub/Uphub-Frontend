...
export default function UCardTopCompany({
  ...
  return (
    <div
      className={`w-80 p-4 ${getStyleCardVariant(variant)} 
    ...
    >
      ...
      <div className={"flex flex-row justify-between items-center w-full"}>
        <h2 className="text-base font-semibold md:text-lg">{companyName}</h2>
      ...
      {/* Button Groups */}
      <div
        className="mt-4 flex flex-col text-sm w-full items-center justify-center
											sm:text-xs sm:flex-row sm:justify-between gap-2 flex-1"
      >
        {isHiring && (
          <div className="rounded-full px-3 py-1 bg-custom-yellow-1 text-custom-blue-2 md:text-sm">Đang Tuyển Dụng</div>
        )}
        {hasHighBenefits && (
          <div className="rounded-full border px-3 py-1 border-custom-blue-3 text-custom-blue-2 md:text-sm">
            Phúc Lợi Cao
          </div>
        )}
      </div>
